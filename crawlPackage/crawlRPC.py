...
class CrawlRPC:
    ...
    def batch_Blocks2Receipts(self, blocks: list) -> list:
        ...
        values = self.batchRequests([("eth_getBlockByNumber", [hex(block), True]) for block in blocks])
        return values
    ...
    @retry_on_failure(max_attempts=5, delay_seconds=10)
    ...
    def ERC20TokenBalanceOf(self, erc20: str, contract: str, block: int):
        ...
        data = "0x70a08231" +   contract.lstrip("0x").rjust(64, "0")
        payload = json.dumps({
        "method": "eth_call",
        "params": [
                {
